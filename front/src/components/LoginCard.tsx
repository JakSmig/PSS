...
const LoginCard = ({ setForm }: any) => {
  ...
  const renderContent = () => {
    if (signup) {
      return (
        <SignUp handleSignIn={setSignup} setForm={setForm}/>
      );
    } else{
      return(
        <Card
    ...
    >
      ...
      >
        <form onSubmit={formik.handleSubmit} style={{margin : "auto 40px"}}>
          <Typography variant="h6" fontWeight="700" fontSize="35px" sx={{ my: 1 }} align="center">
            Log in
          </Typography>
          ...
          <Button type="submit" variant="contained" sx={{ width: "100%", my: 3 }}>
            Log in
          </Button>
          </Box>
        </form>
        ...
        <Typography><Button onClick={()=>{setSignup(true); console.log(signup)}}>Sign up </Button>if you dont't have an account yet</Typography>
      </Grid>
    </Card>
